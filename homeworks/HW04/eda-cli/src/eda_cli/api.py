...
async def get_quality_flags_from_csv(file: UploadFile = File(...)) -> dict:
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail=f"Не удалось прочитать CSV: {read_error}"
        )
    ...
    dataset_summary = summarize_dataset(data_frame)
    missing_data = missing_table(data_frame)
    all_quality_flags = compute_quality_flags(dataset_summary, missing_data)

    processing_time_ms = (perf_counter() - timer_start) * 1000.0

    boolean_flags = {
        flag_name: bool(flag_value)
        for flag_name, flag_value in all_quality_flags.items()
